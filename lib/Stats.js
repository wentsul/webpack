...
const outputChildStats = stats => {
	const obj = stats.toJson("verbose");
	console.log(`-------------- start ---------`);
	console.log(`outputting child stats for ${stats.name}`);
	console.log(`hasErrors ${stats.hasErrors()}`);
	console.log(`errors ${makeJson(obj.errors)}`);
	console.log(`warnings ${makeJson(obj.warnings)}`);
	console.log("outputting modules");
	console.log(`stats keys: ${Object.keys(stats)}`);
	console.log(`obj keys: ${Object.keys(obj)}`);
	// obj.modules.forEach(module => {
	// 	console.log(`module name: ${module.name}`);
	// 	console.log(`module failed: ${module.failed}`);
	// 	console.log(`number of errors: ${module.errors}`);
	// 	console.log(`number of warnings: ${module.warnings}`);
	// });
	console.log(`-------------- end ---------`);
};
...
class Stats {
	...
	hasErrors() {
		console.log(`hasErrors called with stat ${this.compilation.name}`);
		console.log(`errors length ${this.compilation.errors.length}`);
		const hasChildError = this.compilation.children.some(child =>
			child.getStats().hasErrors()
		);
		console.log(`hasChildError: ${hasChildError}`);
		this.compilation.children.forEach(child => {
			const obj = child.getStats();
			const hasErrors = obj.hasErrors();
			console.log(
				`hasErrors called with stat ${this.compilation.name}: ${hasErrors}`
			);
			console.log(`outputting child stats: ${outputChildStats(obj)}`);
		});
		const hasErrors =
			this.compilation.errors.length > 0 ||
			this.compilation.children.some(child => child.getStats().hasErrors());
		console.log(`hasErrors for stat ${this.compilation.name}: ${hasErrors}`);
		return hasErrors;
	}
