...
const outputChildStats = stats => {
	...
	console.log(`   -------------- start child output ---------`);
	...
	obj.assets.forEach(asset => {
		...
		console.log(`asset emitted: ${asset.emitted}`);
	});
	obj.assets.forEach(asset => {
		console.log(`asset name: ${asset.name}`);
		console.log(`asset size: ${asset.size}`);
...
class Stats {
	...
	hasErrors() {
		console.log(
			`------------- start hasErrors called for stat ${
				this.compilation.name
			} ---------`
		);
